...
const createServer = () => {
  ...
  app.get('/aboutme' , (request, response) => {
    response.send('I have 2 dogs and one likes to play football all the time');
  });

  app.get('/favoritefoods', (request, response) => {
    response.json(['steak, pizza, sweet potatoes']);
  });
...
const count = (target, input) => {
  ...
  return input.reduce((accumulator, value) =>{
    return accumulator + value.reduce((accumulator, value) => {
     if(value === target)  {
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
 return input.map(x => x.filter(x => typeof x === 'number' && x % 5 === 0).map(x => Math.pow(2,x)));
